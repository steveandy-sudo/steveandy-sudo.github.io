...
현재 상세 페이지의 공개 예약 슬롯 ID: `uav-demo`. `mobility-waypoint`와 `mobility-aeb-stop`에는 실제 GIF와 포스터를 연결했습니다. 편집 구간과 출처는 [MEDIA_SOURCES.md](MEDIA_SOURCES.md)에 기록했습니다. V2I는 홈페이지에 설계 배치도 자리를 예약합니다. 국민대 6초 주행 영상과 접촉 장면을 제외한 주차 영상은 현재 편집본을 유지합니다.
