...
현재 상세 페이지의 `uav-demo`, `mobility-waypoint`, `mobility-aeb-stop`에는 실제 GIF와 포스터를 연결했습니다. 편집 구간과 출처는 [MEDIA_SOURCES.md](MEDIA_SOURCES.md)에 기록했습니다. V2I는 홈페이지에 4주차 비교 구조를, 상세 페이지에 시나리오·비교 설계 슬라이드를 표시합니다. 국민대 6초 주행 영상과 접촉 장면을 제외한 주차 영상은 현재 편집본을 유지합니다.
